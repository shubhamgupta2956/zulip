...
function assert_not_mark_read_with_has_operands(additional_operators_to_test) {
    ...
    var has_operator = [{ operator: 'has', operand: 'link' }];
    var filter = new Filter(additional_operators_to_test.concat(has_operator));
    assert(!filter.can_mark_messages_read());
...
function assert_not_mark_read_with_is_operands(additional_operators_to_test) {
    additional_operators_to_test = additional_operators_to_test || [];
    var is_operator = [{ operator: 'is', operand: 'starred' }];
    var filter = new Filter(additional_operators_to_test.concat(is_operator));
    assert(!filter.can_mark_messages_read());
...
function assert_not_mark_read_when_searching(additional_operators_to_test) {
    additional_operators_to_test = additional_operators_to_test || [];
    var search_op = [{ operator: 'search', operand: 'keyword' }];
    var filter = new Filter(additional_operators_to_test.concat(search_op));
    assert(!filter.can_mark_messages_read());
...
run_test('can_mark_messages_read', () => {
    ...
    var filter = new Filter(stream_operator);
    assert(filter.can_mark_messages_read());
    ...
    var stream_negated_topic_operators = [
        { operator: 'stream', operand: 'foo' },
